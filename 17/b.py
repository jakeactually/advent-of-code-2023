...
def solve():
    ...
    while queue:
        ...
        # 1. Continue in the same direction if consecutive_steps < 10
        if consecutive_steps < 10:
            nr, nc = r + dr, c + dc
            if 0 <= nr < height and 0 <= nc < width:
                heapq.heappush(queue, (heat_loss + grid[nr][nc], nr, nc, dr, dc, consecutive_steps + 1))
                
        # 2. Turn 90 degrees (left or right) - only if we have moved at least 4 steps in the current direction
        if consecutive_steps >= 4:
